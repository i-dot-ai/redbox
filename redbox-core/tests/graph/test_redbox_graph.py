...
from tests.conftest import all_chunks_retriever
...
@pytest.mark.parametrize(
    ("test_case"),
    [
        test_case for generated_cases in [
            generate_test_cases(
            ...
            generate_test_cases(
                query=ChainInput(question="What is AI?", file_uuids=[uuid4()], user_uuid=uuid4(), chat_history=[]),
                test_data=[
                    TestData(2, 200_000, ["Intermediate document"]*2 + ["Testing Response 1"], ChatRoute.chat_with_docs),
                    TestData(5, 80000, ["Intermediate document"]*5 + ["Testing Response 1"], ChatRoute.chat_with_docs),
                    TestData(10, 100_000, ["Intermediate document"]*10 + ["Testing Response 1"], ChatRoute.chat_with_docs)
                ]
            ),
...
async def test_chat(test_case: RedboxChatTestCase, env, tokeniser):
    app = get_redbox_graph(
        llm=FakeStreamingListLLM(responses=test_case.llm_response),
    ...
    assert final_state["response"] == test_case.llm_response[-1], f"Expected LLM response: '{test_case.llm_response[-1]}'. Received '{final_state["response"]}'"
    assert final_state["route_name"] == test_case.expected_route, f"Expected Route: '{ test_case.expected_route}'. Received '{final_state["route_name"]}'"
...
async def test_streaming_chat(test_case: RedboxChatTestCase, env, tokeniser):
    app = get_redbox_graph(
    ...
    final_state = ChainState(response)
    assert len(token_events) > 0, f"Expected tokens as a stream. Received: {token_events}"
    llm_response = "".join(token_events)
    assert final_state["response"] == llm_response, f"Expected LLM response: '{llm_response}'. Received '{final_state["response"]}'"
    assert final_state["route_name"] == test_case.expected_route, f"Expected Route: '{ test_case.expected_route}'. Received '{final_state["route_name"]}'"