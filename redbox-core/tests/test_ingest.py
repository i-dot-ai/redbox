...
def make_file_query(user_uuid: UUID, file_uuid: UUID, resolution: ChunkResolution | None = None) -> dict[str, Any]:
    query_filter = make_query_filter(user_uuid, [file_uuid], resolution)
...
async def test_ingest_from_loader(
    mock_post: MagicMock,
    ...
    _ = await ingest_chain.ainvoke(file)

    # Test it's written to Elastic
    file_query = make_file_query(user_uuid=file.creator_user_uuid, file_uuid=file.uuid, resolution=resolution)
