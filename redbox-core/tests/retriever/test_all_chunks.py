...
def test_all_chunks_retriever(all_chunks_retriever: AllElasticsearchRetriever, stored_file_all_chunks: RedboxChatTestCase):
    result = all_chunks_retriever.invoke(
        ChainState(query=stored_file_all_chunks.query)
    )
