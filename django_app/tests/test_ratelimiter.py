...
def generate_user_activity(
    state_definitions: list[UserActivity]
):
    """
    Creates a list of RedboxStates representing a user request based on the definitions passed in. There will be a state for 
    each request with the total document tokens split evenly across each users requests
    """
    all_user_requests = [
        [
        RedboxState(
...
def test_ratelimiter(initial_user_credits: int, user_activity_states: list[RedboxState], ratelimit_should_be_triggered: bool):
    ...
    for i, user_activity_state in enumerate(user_activity_states):
        if not ratelimiter.is_allowed(user_activity_state):
            if not ratelimit_should_be_triggered:
                pytest.fail(reason=f"Ratelimit was unexpectedly triggered by user request [{i}] - {user_activity_state.model_dump()}")
            break
    else:
