        ...
        if not len(errors["upload_doc"]):
            ...
            try:
                ...
                file = models.File.objects.create(
                    processing_status=models.ProcessingStatusEnum.uploaded.value,
                    user=request.user,
...
def remove_doc_view(request, doc_id: str):
    file = models.File.objects.get(pk=doc_id)
...
def sessions_view(request, session_id: str = ""):
    ...
    messages = []
    if session_id:
        messages = models.ChatMessage.objects.filter(chat_history__id=session_id)
