...
# Apply preprocessing
df.loc[:, 'processed_text'] = df['text'].apply(preprocess)
...
X = model.encode(df['processed_text'].tolist(), convert_to_tensor=True)
...
df.loc[:, 'task_cluster'] = kmeans.fit_predict(X)

# Summarize the most common tasks
common_tasks = df['task_cluster'].value_counts()
...
for cluster in common_tasks.index:
    ...
    print(df[df['task_cluster'] == cluster]['text'].head(5))
    print("\n")
...
cluster_data = prepare_for_lda(df)
