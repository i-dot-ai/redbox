...
tf_auto_apply: ## Auto apply terraform
	make tf_set_workspace && \
	terraform -chdir=./infrastructure/aws apply -auto-approve -lock-timeout=300s -var-file=$(CONFIG_DIR)/${env}-input-params.tfvars -var=image_tag=$(IMAGE_TAG) ${tf_build_args} -target="module.django-app" -target="module.core-api" -target="module.worker"
