...
from redbox.models.file import Chunk, encoding
...
def reduce_chunks_by_tokens(chunks: list[Document] | None, chunk: Document, max_tokens: int) -> list[Document]:
    ...
    last_chunk_tokens = last_chunk.metadata["_source"]["metadata"].get('token_count') or len(encoding.encode(last_chunk.page_content))
