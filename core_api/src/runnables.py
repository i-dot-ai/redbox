...
def map_es_documents(docs: Iterator[Document]) -> Iterator[Chunk]:
    for d in docs:
        yield Chunk(
            text=d.page_content,
            uuid=d.metadata['_id'],
            **d.metadata["_source"],
            **d.metadata['_source']['metadata']
        )
